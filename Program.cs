    ...
    public class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            while(doneInstalling)
            {
                ...
                GetModsList();
                Console.Write("\n");
                ConsoleC.WriteColors("Type the number of the mod that you want to add, and enter I or U to Install and Uninstall: ", ConsoleColor.DarkCyan, ConsoleColor.Black);
                        ...
                        case "I":
                            {
                                ...
                                ApplyMod(ModsAvailable[ModChosen]);
                                break;
                            }
                        case "U":
                            {
        ...
        {
            ExtractCPK();
            string[] files = CPKSettingsFile.Files;
            ...
            {
                var actualPath = fileArray[i].Replace($"{cache}\\sonic2010_0\\", "");
        ...
        private static void ApplyMod(string path)
        {
            ExtractCPK();
            string extractedCPK = $"{ProgramPath}\\sonic2010_0";
            if (CPKSettingsFile.Files.Length == 0)
                CPKSettingsFile.Files = Directory.GetFiles(path: Path.Combine(ProgramPath, "sonic2010_0"), searchPattern: "*.*", searchOption: SearchOption.AllDirectories);
            ...
            for (int a = 0; a < ModsInstalled.Count; a++)
            {
                ...
                { ConsoleC.WriteLineColors($"Mod {ModsInstalled[a]} doesn't have any files to replace, skipping...", ConsoleColor.Black, ConsoleColor.White); continue; }
                ...
                {
                    var actualPath = fileArray[b].Replace($"{ModsInstalled[a]}\\sonic2010_0\\", "");
                    actualPath = actualPath.Replace(@"\\sonic2010_0\\", "");
                    //If it is actually real, copy the old one to a cache and move the new one in its place
                    if (CPKSettingsFile.Files.Contains(actualPath))
                    {
